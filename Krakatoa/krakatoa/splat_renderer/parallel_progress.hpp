...
class parallel_render_progress_logger_master {
    ...
    std::atomic<std::size_t> m_curTicks;
    ...
    typedef std::function<void( frantic::graphics2d::framebuffer<frantic::graphics::color6f>& )> watermark_type;
  ...
  public:
    ...
    void update_progress( std::size_t tickCount ) {
        std::size_t curTicks = tickCount + m_curTicks.fetch_add( tickCount );

        if( std::this_thread::get_id() == m_loggingThreadId )
    ...
    void update_progress_and_image( std::size_t tickCount,
        ...
        if( std::this_thread::get_id() == m_loggingThreadId && relativeOrdering <= m_imageOrdering ) {
  ...
  public:
    parallel_render_progress_logger_master( boost::shared_ptr<frantic::logging::render_progress_logger> delegateLogger,
        ...
        : m_loggingThreadId( std::this_thread::get_id() )
